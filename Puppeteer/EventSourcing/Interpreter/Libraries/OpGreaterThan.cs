...
{
	class OpGreaterThan : BinaryAstExpression
	{
		internal OpGreaterThan(AstExpression e1, AstExpression e2) : base(e1, e2)
		{
		}
		...
		internal override Expression ExecuteExpression(ParameterExpression parametersParam)
		{
			...
			if (leftType == typeof(int) && rightType == typeof(decimal))
				return Expression.GreaterThan(Expression.Convert(left, typeof(decimal)), right);

			if (leftType == typeof(double) && rightType == typeof(int))
				return Expression.GreaterThan(left, Expression.Convert(right, typeof(double)));
			...
			if (leftType == typeof(decimal) && rightType == typeof(int))
				return Expression.GreaterThan(left, Expression.Convert(right, typeof(decimal)));
